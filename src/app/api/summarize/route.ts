...
export async function POST(req: NextRequest) {
  ...
  if (!provCfg || !(provCfg.models as readonly string[]).includes(model)) {
    return Response.json({ error: `Unsupported model '${modelUsed}'.` }, { status: 400 });
  }
  ...
  const summary = await summarizeThread({
    ...
    messages: msgs.map((m) => ({ id: m.id, role: m.role as any, content: m.content, pinned: m.pinned })),
    model: modelUsed,
  });
  ...
  await patchThread(thread.id, {
    ...
    summaryJson: summary as unknown as any,
  });
  return Response.json({ ...summary, model: modelUsed, updatedAt: updatedAt.toISOString() });
}
