...
import { createMessage, listMessages, patchMessage, deleteMessage, getThread } from "../../../db/actions";
...
export async function POST(req: NextRequest) {
  ...
  if (!thr) return Response.json({ error: "Thread not found" }, { status: 404 });
  ...
  return Response.json(MessageSchema.parse(serializeMessage(m)), { status: 201 });
}
...
export async function GET(req: NextRequest) {
  ...
  const db = getDb();
  const where = [eq(messages.threadId, threadId)] as any[];
  if (cursor) {
    const createdAt = new Date(cursor.createdAt);
    // older than the cursor
    where.push(or(lt(messages.createdAt, createdAt), and(eq(messages.createdAt, createdAt), lt(messages.id, cursor.id))));
  }
  ...
  const nextCursor = rowsDesc.length > limit ? makeCursor(rowsDesc[rows.length]) : null;
  // Return ascending for UI readability
  const page = rows.reverse().map(serializeMessage);
  return Response.json({ items: page.map((m) => MessageSchema.parse(m)), nextCursor });
}

const PatchQuery = z.object({ id: z.string() });
const PatchBody = z.object({ pinned: z.boolean().optional() });
