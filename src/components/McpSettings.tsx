...
export function McpSettingsModal({ initialOpen = false }: { initialOpen?: boolean }) {
  const [open, setOpen] = useState(initialOpen);
  const { config, addServer, removeServer, toggleFeature } = useMcpServers();
  if (!config.featureEnabled) return null;
  return (
    <div>
      <Button size="sm" onClick={() => setOpen(true)}>MCP Settings</Button>
      {open && (
          ...
          <Card className="w-full max-w-lg p-4">
            ...
            <div className="mt-3 space-y-3">
              <label className="inline-flex items-center gap-2 text-sm">
                <input type="checkbox" checked={config.featureEnabled} onChange={(e) => toggleFeature(e.target.checked)} />
                Enable MCP
              </label>
                ...
                {config.servers.map((s) => (
                  <div key={s.id} className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="truncate text-xs text-[color:var(--color-muted)]">{s.endpoint}</div>
                    </div>
                    <Button size="sm" onClick={() => removeServer(s.id)}>Remove</Button>
                  </div>
                ))}
...
function AddServer({ onAdd }: { onAdd: (s: any) => void }) {
  ...
  return (
    ...
    >
      <Input placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} className="max-w-[140px]" />
