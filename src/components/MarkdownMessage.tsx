...
let highlighterPromise: Promise<
  shiki.HighlighterGeneric<shiki.BundledLanguage, shiki.BundledTheme>
> | null = null;
const SUPPORTED_LANGS = new Set<string>(
  Object.keys(shiki.bundledLanguages).map((k) => k.toLowerCase()),
);
...
function resolveLang(input: string | undefined): {
  safe: shiki.BundledLanguage | "plaintext";
  display: string;
  canon: string;
} {
  const raw = (input ?? "").toLowerCase();
  const canon = LANG_ALIASES[raw] ?? raw;
  if (SUPPORTED_LANGS.has(canon))
    return { safe: canon as shiki.BundledLanguage, display: canon, canon };
...
function getHighlighter() {
  if (!highlighterPromise) {
    ...
    highlighterPromise = createBundled({
      ...
      langs: LANGS_TO_LOAD as unknown as shiki.BundledLanguage[],
    });
  }
  ...
  useEffect(() => {
    ...
    timerRef.current = window.setTimeout(async () => {
      try {
        ...
        const theme =
          themeAttr === "dark" ? "github-dark-default" : "github-light-default";
        let htmlOut = highlighter.codeToHtml(raw, {
          lang: safeLang as shiki.BundledLanguage,
          theme,
        });
...
export function MarkdownMessage({ content }: MarkdownMessageProps) {
  ...
  const components: Components = {
    ...
    th: ({ children }) => (
      <th className="border border-[color:var(--color-border)] px-2 py-1 text-left">
        {children}
      </th>
    ),
    td: ({ children }) => (
      <td className="border border-[color:var(--color-border)] px-2 py-1 align-top">
        {children}
      </td>
    ),
    ...
    code: (rawProps) => {
      ...
      const { inline, children, className, ...props } =
        rawProps as CodePropsLike;
      const lang = (className || "")
        .match(/language-([\w-]+)/)?.[1]
        ?.toLowerCase();
      const proseLangs = new Set([
        "markdown",
        "md",
        "mdx",
        "text",
